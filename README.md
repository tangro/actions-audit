...
You can use a specific `version` of this action. The latest published version is `v2.0.23`. You can also use `latest` to always get the latest version.
...
audit:
  ...
  steps:
    ...
    - name: Run npm install
      run: npm install
    - name: Run audit
      uses: tangro/actions-audit@v2.0.23
      env:
